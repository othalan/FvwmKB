...
"""Copyright (C) 1996-2001 Barry A. Warsaw <barry@python.org>

FVWM version 2 module interface framework for Python.

Note, Fvwm modules cannot be run from the command line.  They must be
...
class FvwmError(Exception):
    """Base class for all errors in this module."""
    pass

class PipeReadError(FvwmError):
    pass

class UsageError(FvwmError):
    pass
...
class PacketReader:
	...
	try:
	    ...
	    return None
        classname = EMPTYSTRING.join(
            [s.capitalize() for s in msgtype.split('_')[1:] + ['Packet']])
	...
	def __init__(self, typenum, fp):
	    ...
	    self.msgtype = typenum
            self.pkthandler = EMPTYSTRING.join(
                [s.capitalize() for s in Types[typenum].split('_')[1:]])
...
class FvwmModule:
	...
	if len(argv) < 6:
	    raise UsageError, \
		  ...
		  argv[0]
	try:
            self.__tofvwm   = os.fdopen(int(argv[1]), 'wb', 0)
            self.__fromfvwm = os.fdopen(int(argv[2]), 'rb', 0)
	except ValueError:
	    raise UsageError, \
		  'Usage: %s ofd ifd configfile appcontext wincontext' % \
		  argv[0]
...
## 	    debug('dispatching: %s' % pkt.pkthandler)
	...
	Types[pktname]
        pkthandler = EMPTYSTRING.join(
            [s.capitalize() for s in pktname.split('_')[1:]])
	try:
	...
	if mask is None:
		...
		if type(name) <> StringType:
		    continue
                methodname = EMPTYSTRING.join(
                    [s.capitalize() for s in name.split('_')[1:]])
...
class ConfigInfo:
	...
	get_infolines(regexp)
            -- returns all lines.  If optional RE is given, only lines that
	       start with `*<RE>' are returned (the initial star should not be
	       included).
	...
	for line in self.__lines:
            parts = line.split()
            if parts[0].lower() == varname:
		return parts[1]

    def get_infolines(self, regexp=None):
	pred = None
	if regexp:
	    cre = re.compile(r'\*' + regexp)
